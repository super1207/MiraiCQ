...
BOOL CStackWalker::LoadSymbol()
{
	...
	if (NULL != m_lpszSymbolPath)
	{
		...
		m_bSymbolLoaded = SymInitialize(m_hProcess, (m_lpszSymbolPath), FALSE);
		return m_bSymbolLoaded;
	}
	...
	if (NULL != m_lpszSymbolPath)
	{
		m_bSymbolLoaded = SymInitialize(m_hProcess, (m_lpszSymbolPath), TRUE); //这里设置为TRUE，让它在初始化符号表的同时加载符号表
	}
...
void CStackWalker::GetModuleInformation(LPMODULE_INFO pmi)
{
	...
	StringCchCopy(pmi->szSymbolPath, MAX_PATH, (im.LoadedPdbName));
}
	...
	while (StackWalk64(imageType, m_hProcess, hThread, &sf, &c, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL))
	{
		...
		if (sf.AddrPC.Offset != 0)
		{
			if(SymGetSymFromAddr64(m_hProcess, sf.AddrPC.Offset, &dwDisplayment, pSym))
			{
				...
				StringCchCopy(pCallStack->szFncName, STACKWALK_MAX_NAMELEN, (pSym->Name));
				UnDecorateSymbolName(pSym->Name, szName, STACKWALK_MAX_NAMELEN, UNDNAME_COMPLETE);
				StringCchCopy(pCallStack->undFullName, STACKWALK_MAX_NAMELEN, (szName));
				...
				StringCchCopy(pCallStack->undName, STACKWALK_MAX_NAMELEN, (szName));
			}else
			{
			...
			{
				StringCchCopy(pCallStack->szFileName, MAX_PATH, (pLine->FileName));
